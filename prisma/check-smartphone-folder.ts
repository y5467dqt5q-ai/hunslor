...
console.log('📱 Проверка папки Smartphone...\n');
console.log(`Путь: ${PATH_SMARTPHONE}`);
console.log(`Существует: ${fs.existsSync(PATH_SMARTPHONE)}\n`);

if (fs.existsSync(PATH_SMARTPHONE)) {
  const folders = fs.readdirSync(PATH_SMARTPHONE, { withFileTypes: true })
  ...
  folders.forEach(folderName => {
    const folderPath = path.join(PATH_SMARTPHONE, folderName);
