...
function extractBrandAndModel(folderName: string): { brand: string; model: string; color?: string } {
  // Извлекаем бренд (Meta или Ray-Ban)
  let brand = 'Meta';
  if (folderName.includes('Ray-Ban')) {
    brand = 'Ray-Ban';
  }
  ...
  // Извлекаем цвет из скобок, если есть (для Ray-Ban)
...
async function main() {
  console.log('🥽 Импорт VR товаров...\n');

  if (!fs.existsSync(PATH_VR)) {
    console.error(`❌ Папка не найдена: ${PATH_VR}`);
    return;
  }

  // Создаем или находим категорию VR
  const category = await prisma.category.upsert({
    ...
    create: {
      name: 'VR',
      slug: 'vr-headsets',
      description: 'VR-Headsets und AR-Brillen',
    },
  });
  console.log(`✅ Категория: ${category.name} (${category.slug})\n`);

  const folders = fs.readdirSync(PATH_VR, { withFileTypes: true })
  ...
  for (const folderName of folders) {
    try {
      const folderPath = path.join(PATH_VR, folderName);
